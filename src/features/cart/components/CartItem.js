...
const CartItem = React.memo(({ item, changeItemQuantity, removeItem }) => {
  ...
  const handleQuantityDec = React.useCallback(() => changeItemQuantity(item, item.quantity - 1), [item, changeItemQuantity]);
  const handleQuantityInc = React.useCallback(() => changeItemQuantity(item, item.quantity + 1), [item, changeItemQuantity]);
