...
export const VirtualItem = React.memo(({
  ...
  const buttonAction = React.useMemo(() => product.virtualPrice ? handleBuyByVC : handleItemAdd, [product, handleBuyByVC, handleItemAdd]);
  const buttonContent = React.useMemo(() => product.virtualPrice ? 'Buy now' : <ShoppingCart />, [product]);
  const price = React.useMemo(() => {
    ...
    if (product.virtualPrice) {
      return (
        <Currency
          image={product.virtualPrice.imageUrl}
          value={product.virtualPrice.amount}
        />
      );
    }
